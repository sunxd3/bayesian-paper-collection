...
parameters {
  ...
  vector<lower=0>[5] sigma_a; // for student-t reparametrisation - https://mc-stan.org/docs/2_29/stan-users-guide/reparameterization.html#reparameterizing-a-student-t-distribution
  vector<lower=0>[5] sigma_b; // for student-t reparametrisation - https://mc-stan.org/docs/2_29/stan-users-guide/reparameterization.html#reparameterizing-a-student-t-distribution
...
transformed parameters{
  ...
  vector<lower=0>[5] sigma = sigma_a ./sqrt(sigma_b); 
  ...
  gam = Phi_approx(mu_pr01[2] + sigma[2] * gam_pr_R)*1;
  eta = mu_pr[1] + sigma[3] * eta_pr_R;
  E0 = mu_pr[2] + sigma[4] * E0_pr_R;
  cs = mu_pr[3] + sigma[5] * cs_pr_R;
  // bet = Phi_approx(mu_pr01[3] + sigma[5] * bet_pr_R)*1;
  // C = mu_pr[3] + sigma[6] * C_pr_R;
}
...
generated quantities{
  ...
  mu_gam = Phi_approx(mu_pr01[2]);
  mu_eta = mu_pr[1];
  mu_E0 = mu_pr[2];
  mu_cs = mu_pr[3];
  // mu_bet = Phi_approx(mu_pr01[3]);
  // mu_C = mu_pr[3];
