...
parameters {
  ...
  // vector[1] mu_pr01;
  vector<lower=0>[6] mu_pr;
  vector<lower=0>[6] sigma_a; // for student-t reparametrisation - https://mc-stan.org/docs/2_29/stan-users-guide/reparameterization.html#reparameterizing-a-student-t-distribution
  vector<lower=0>[6] sigma_b; // for student-t reparametrisation - https://mc-stan.org/docs/2_29/stan-users-guide/reparameterization.html#reparameterizing-a-student-t-distribution
...
transformed parameters{
  ...
  // vector<lower=0, upper=50>[N] eps; //observation noise for the posterior noxious input
  ...
  vector<lower=0>[6] sigma = sigma_a ./sqrt(sigma_b); 
  ...
  // eps = Phi_approx(mu_pr[1] + sigma[1] * eps_pr_R)*50;
  psi = mu_pr[1] + sigma[1] * psi_pr_R;
  eta = mu_pr[2] + sigma[2] * eta_pr_R;
  xi = mu_pr[3] + sigma[3] * xi_pr_R;
  E0 = mu_pr[4] + sigma[4] * E0_pr_R;
  w0 = mu_pr[5] + sigma[5] * w0_pr_R;
  cs = mu_pr[6] + sigma[6] * cs_pr_R;
  // bet = Phi_approx(mu_pr01[1] + sigma[6] * bet_pr_R)*1;
  // C = mu_pr[6] + sigma[7] * C_pr_R;
